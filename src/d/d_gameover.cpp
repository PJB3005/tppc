...
dDlst_GameOverScrnDraw_c::dDlst_GameOverScrnDraw_c(JKRArchive* i_archive) {
    ...
    if (dMeter2Info_getGameOverType() != 0) {
        mpScreen->search(TAG("n_base"))->hide();
    ...
    mpScreen->search(TAG("base_b"))->hide();
    ...
    ResTIMG* img = (ResTIMG*)dComIfGp_getMain2DArchive()->getResource(TAG("TIMG"), "tt_block8x8.bti");
    mpBackImg =
        new J2DPicture(TAG("PICT01"), JGeometry::TBox2<f32>(0.0f, 486.0f, 0.0f, 660.0f), img, NULL);
    mpBackImg->setBlackWhite(img_white, img_black);

    J2DTextBox* gold_tbox = (J2DTextBox*)mpScreen->search(TAG("gold_00"));
