...
dMsgScrnItem_c::dMsgScrnItem_c(u8 param_1, u8 param_2, JKRExpHeap* param_3) {
    ...
    field_0x178 = field_0x0e0[0]->width * mpScreen->search(TAG("set_it_n"))->getWidth() / 48.0f;
    field_0x17c = field_0x0e0[0]->height * mpScreen->search(TAG("set_it_n"))->getHeight() / 48.0f;
    field_0x170 = mpScreen->search(TAG("set_it_n"))->getWidth();
    field_0x174 = mpScreen->search(TAG("set_it_n"))->getHeight();
    for (int i = 0; i < 3; i++) {
    ...
    field_0x160 = mpScreen->search(TAG("n_all"))->getBounds().i.x;
    field_0x164 = mpScreen->search(TAG("n_all"))->getBounds().i.y;
    field_0x180 = 0.0f;
    field_0x184 = 0.0f;
    for (int i = 0; i < 3; i++) {
    ...
    if (dComIfGs_getOptRuby() == 0) {
        ...
        mpTm_c[2] = new CPaneMgr(mpTxScreen, TAG("t3f_w"), 0, NULL);
        ...
        mpTmr_c[2] = new CPaneMgr(mpTxScreen, TAG("mg_3f_w"), 0, NULL);
        ...
        JUT_ASSERT(410, mpTm_c[1] != NULL);
        mpTm_c[2] = new CPaneMgr(mpTxScreen, TAG("t3_w"), 0, NULL);
        JUT_ASSERT(413, mpTm_c[2] != NULL);
    ...
    JUT_ASSERT(410, mpTm_c[1] != NULL);
    mpTm_c[2] = new CPaneMgr(mpTxScreen, TAG("t4_w"), 0, NULL);
    JUT_ASSERT(413, mpTm_c[2] != NULL);
    mpTxScreen->search(TAG("n_3line"))->hide();
    mpTxScreen->search(TAG("n_3fline"))->hide();
...
void dMsgScrnItem_c::drawSelf() {
    ...
    for (int i = 0; i < 3; i++) {
        if (mpItemPane[i] != NULL) {
            ...
            mpItemPane[i]->draw(mpScreen->search(TAG("set_it_n"))->getGlbBounds().i.x +
                                    0.5f * (field_0x170 - (field_0x178 * texScale)) +
                                    g_MsgObject_HIO_c.mBoxItemPosX,
                                mpScreen->search(TAG("set_it_n"))->getGlbBounds().i.y +
...
void dMsgScrnItem_c::fukiPosCalc(u8 param_1) {
    ...
    case 1:
        ...
        field_0x180 = 105.0f - (mpScreen->search(TAG("n_all"))->getBounds().i.y +
                              mpScreen->search(TAG("n_all"))->getBounds().f.y) / 2;
        mpSelect_c->translate(486.0f, 230.0f);
        break;
    case 2:
        yOffset = g_MsgObject_HIO_c.mBoxPos[1][3];
        field_0x180 = 235.0f - (mpScreen->search(TAG("n_all"))->getBounds().i.y +
                                mpScreen->search(TAG("n_all"))->getBounds().f.y) / 2;
    ...
    mpScreen->search(TAG("n_all"))->move(field_0x160, yOffset + (field_0x164 + field_0x180));
    if (mpPmP_c != NULL) {
        mpPmP_c->reinit();
...
void dMsgScrnItem_c::setBtk0Animation(J2DAnmTextureSRTKey* param_1) {
    mpScreen->search(TAG("gold00"))->setAnimation(param_1);
    mpScreen->search(TAG("gold01"))->setAnimation(param_1);
}

void dMsgScrnItem_c::setBpk0Animation(J2DAnmColor* param_0) {
    mpScreen->search(TAG("map0"))->setAnimation(param_0);
}

void dMsgScrnItem_c::setBpk1Animation(J2DAnmColor* param_0) {
    mpScreen->search(TAG("moyou_1"))->setAnimation(param_0);
}
