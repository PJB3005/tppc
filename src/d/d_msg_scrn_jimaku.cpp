...
dMsgScrnJimaku_c::dMsgScrnJimaku_c(u8 param_0, JKRExpHeap* i_heap) {
    ...
    mpPmP_c = new CPaneMgr(mpScreen, TAG("mg_null"), 3, NULL);
    ...
    if (dComIfGs_getOptRuby() == 0) {
        mpTm_c[0] = new CPaneMgr(mpScreen, TAG("mg_3flin"), 0, NULL);
        mpTm_c[1] = new CPaneMgr(mpScreen, TAG("t3f_s"), 0, NULL);

        mpTmr_c[0] = new CPaneMgr(mpScreen, TAG("mg_3f"), 0, NULL);
        mpTmr_c[1] = new CPaneMgr(mpScreen, TAG("mg_3f_s"), 0, NULL);

        mpScreen->search(TAG("n_3line"))->hide();
        mpScreen->search(TAG("n_3fline"))->show();
        mpScreen->search(TAG("n_e4line"))->hide();
    } else {
        mpTm_c[0] = new CPaneMgr(mpScreen, TAG("mg_3line"), 0, NULL);
        mpTm_c[1] = new CPaneMgr(mpScreen, TAG("t3_s"), 0, NULL);

        mpScreen->search(TAG("n_3line"))->show();
        mpScreen->search(TAG("n_3fline"))->hide();
        mpScreen->search(TAG("n_e4line"))->hide();
    }
#else
    mpTm_c[0] = new CPaneMgr(mpScreen, TAG("mg_e4lin"), 0, NULL);
    JUT_ASSERT(0, mpTm_c[0] != NULL);

    mpTm_c[1] = new CPaneMgr(mpScreen, TAG("t4_s"), 0, NULL);
    JUT_ASSERT(0, mpTm_c[1] != NULL);

    mpScreen->search(TAG("n_3line"))->hide();
    mpScreen->search(TAG("n_3fline"))->hide();
    mpScreen->search(TAG("n_e4line"))->show();
#endif
