...
dMsgScrnArrow_c::dMsgScrnArrow_c() {
    ...
    mpParent_c = new CPaneMgr(mpScreen, TAG("set_ya_n"), 0, NULL);
    JUT_ASSERT(0, mpParent_c != NULL);

    mpArw_c = new CPaneMgr(mpScreen, TAG("ya_next"), 0, NULL);
    JUT_ASSERT(0, mpArw_c != NULL);

    mpArw_c->hide();
    mpArw_c->mPane->setAnimation(mpBck);
    mpScreen->search(TAG("yajnext"))->setAnimation(mpBpk);
    mpScreen->search(TAG("yajinexl"))->setAnimation(mpBpk);

    mpDot_c = new CPaneMgr(mpScreen, TAG("ya_end"), 0, NULL);
    JUT_ASSERT(0, mpDot_c != NULL);

    mpDot_c->hide();
    mpScreen->search(TAG("yaj_end"))->setAnimation(mpBpk);
    mpScreen->search(TAG("yajiendl"))->setAnimation(mpBpk);
}
