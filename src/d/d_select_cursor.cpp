...
dSelect_cursor_c::dSelect_cursor_c(u8 param_0, f32 param_1, JKRArchive* param_2) {
    ...
    static u64 const corner_tag[4] = {TAG("l_u_null"), TAG("l_d_null"), TAG("r_u_null"), TAG("r_d_null")};

    switch(mNameIdx) {
        case 0:
            mpPaneMgr = new CPaneMgr(mpScreen, TAG("n_all"), 2, NULL);
            mpPaneMgr->hide();
        ...
        case 1:
            ...
            mpPaneMgr->setAlpha(0);
            mpPaneMgr->scale(param_1, param_1);
            field_0x14 = new CPaneMgr(mpScreen, TAG("sel_po00"), 0, NULL);
            field_0x14->hide();
            field_0x18 = new CPaneMgr(mpScreen, TAG("n_all2"), 2, NULL);
            break;
        case 2:
            mpPaneMgr = new CPaneMgr(mpScreen, TAG("Null"), 2, NULL);
            mpPaneMgr->hide();
            mpPaneMgr->setAlpha(0);
            mpPaneMgr->scale(param_1, param_1);
            field_0x14 = NULL;
            ...
            break;
        case 3:
            mpPaneMgr = new CPaneMgr(mpScreen, TAG("batsu"), 2, NULL);
            mpPaneMgr->hide();
            mpPaneMgr->setAlpha(0);
            mpPaneMgr->scale(param_1, param_1);
            field_0x14 = NULL;
    ...
    switch(mNameIdx) {
        case 1:
            field_0x50 = mpScreen->search(TAG("ssel_ico"))->getTranslateX();
            field_0x54 = mpScreen->search(TAG("ssel_ico"))->getTranslateY();
            break;
        case 0:
        case 2:
        case 3:
...
void dSelect_cursor_c::setBpkAnimation(J2DAnmColor* param_0) {
    switch (mNameIdx) {
    case 0: {
        static u64 const tag_4181[4] = {TAG("l_ld"), TAG("l_lu"), TAG("l_rd"), TAG("l_ru")};
        for (int i = 0; i < 4; i++) {
    ...
    case 2:
        mpScreen->search(TAG("light"))->setAnimation(param_0);
        mpScreen->search(TAG("back_l"))->setAnimation(param_0);
        break;
    case 3:
        mpScreen->search(TAG("batsu"))->setAnimation(param_0);
        break;
    default:
        mpScreen->search(TAG("light00"))->setAnimation(param_0);
        break;
    }
}
...
void dSelect_cursor_c::setBtk0Animation(J2DAnmTextureSRTKey* param_0) {
    ...
    case 0: {
        static u64 const tag_4197[8] = {
            TAG("i_c_ld1"), TAG("i_c_ld2"), TAG("i_c_lu1"), TAG("i_c_lu2"), TAG("i_c_rd1"), TAG("i_c_rd2"), TAG("i_c_ru1"), TAG("i_c_ru2"),
        };
        for (int i = 0; i < 8; i++) {
    ...
    case 1: {
        static u64 const tag_4204[2] = {TAG("gold01"), TAG("gold02")};
        for (int i = 0; i < 2; i++) {
    ...
    default:
        mpScreen->search(TAG("gold01"))->setAnimation(param_0);
        break;
    }
}
...
void dSelect_cursor_c::setCursorAnimation() {
    ...
    moveCenter(mpScreen->search(TAG("l_u_null")), field_0x74[0], field_0x84[0]);
    moveCenter(mpScreen->search(TAG("l_d_null")), field_0x74[1], field_0x84[1]);
    moveCenter(mpScreen->search(TAG("r_u_null")), field_0x74[2], field_0x84[2]);
    moveCenter(mpScreen->search(TAG("r_d_null")), field_0x74[3], field_0x84[3]);
}


void dSelect_cursor_c::setBckAnimation(J2DAnmTransformKey* param_0) {
    mpScreen->search(TAG("ssel_ico"))->setAnimation((J2DAnmTransform*)param_0);
}
