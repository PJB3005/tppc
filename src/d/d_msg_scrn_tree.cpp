...
dMsgScrnTree_c::dMsgScrnTree_c(JUTFont* param_0, JKRExpHeap* param_1) {
    ...
    mpPmP_c = new CPaneMgr(mpScreen, TAG("n_size"), 2, NULL);
    ...
    field_0xc4 = new CPaneMgr(mpScreen, TAG("back_b"), 0, NULL);
    field_0xc8 = new CPaneMgr(mpScreen, TAG("spot00"), 0, NULL);
    field_0xc8->getPanePtr()->setAnimation(field_0xd0);

    mpScreen->search(TAG("white_m"))->setAnimation(field_0xd4);

#if VERSION == VERSION_GCN_JPN
    if (dComIfGs_getOptRuby() != 0) {
        for (int i = 0; i < 3; i++) {
            static u64 const t_tag[3] = {TAG("mg_3line"), TAG("t3_w"), TAG("t3_s")};
        ...
        for (int i = 0; i < 3; i++) {
            static u64 const t_tag[3] = {TAG("t3fline"), TAG("t3f_w"), TAG("t3f_s")};
            static u64 const tr_tag[3] = {TAG("mg_3f"), TAG("mg_3f_w"), TAG("mg_3f_s")};
...
#else
    for (int i = 0; i < 3; i++) {
        static u64 const t_tag[3] = {TAG("mg_e4lin"), TAG("f4_w"), TAG("t4_s")};
