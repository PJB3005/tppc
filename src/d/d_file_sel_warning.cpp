...
void dFile_warning_c::screenSet() {
    ...
    mpRootPane = new CPaneMgr(mFileWarn.Scr, TAG("Nm_02"), 0, NULL);
    JUT_ASSERT(0, mpRootPane != NULL);
    field_0x34 = mpRootPane->getTranslateY();

#if REGION_JPN
    mFileWarn.Scr->search(TAG("ms_for_2"))->hide();
    mFileWarn.Scr->search(TAG("ms_for_3"))->hide();

    field_0x20 = static_cast<J2DTextBox*>(mFileWarn.Scr->search(TAG("w_msg_jp")));
#else
    mFileWarn.Scr->search(TAG("w_msg_jp"))->hide();
    mFileWarn.Scr->search(TAG("ms_for_2"))->hide();

    field_0x20 = static_cast<J2DTextBox*>(mFileWarn.Scr->search(TAG("ms_for_3")));
#endif
    field_0x20->show();
